...
//-------------------------------------------------------------------------------------------------------------------
// Switch the PLL source from MSI to HSE, and select the PLL as SYSCLK source.
// 
// (DMH - Assumes HSE external clock is provided by the ST-LINK MCO output at 8 MHz)
//-------------------------------------------------------------------------------------------------------------------
void System_Clock_Init_HSE(void){
	
/// DMH --- RC Calibration not used for HSE source
//	uint32_t HSITrim;
  ...
  // (HCLK) and the supply voltage of the device.		
	...
	RCC->CR |= RCC_CR_HSEON;                //// DMH (was HSION)
	while((RCC->CR & RCC_CR_HSERDY) == 0);  //// DMH (HSIRDY
...
// 	RCC->ICSCR |= HSITrim << 24;
	...
	RCC->PLLCFGR |= RCC_PLLCFGR_PLLSRC_HSE; // 00 = No clock, 01 = MSI, 10 = HSI, 11 = HSE  /////// DMH (Was HSI)
	
	// Make PLL as 80 MHz
	// f(VCO clock) = f(PLL clock input) * (PLLN / PLLM) = 16MHz * 20/2 = 160 MHz
	// f(PLL_R) = f(VCO clock) / PLLR = 160MHz/2 = 80MHz
	RCC->PLLCFGR = (RCC->PLLCFGR & ~RCC_PLLCFGR_PLLN) | 20U << 8;
	RCC->PLLCFGR &= ~RCC_PLLCFGR_PLLM; // 000: PLLM = 1, 001: PLLM = 2, 010: PLLM = 3, 011: PLLM = 4, 100: PLLM = 5, 101: PLLM = 6, 110: PLLM = 7, 111: PLLM = 8  ////// DMH was (RCC->PLLCFGR & ~RCC_PLLCFGR_PLLM) | 1U << 4;
